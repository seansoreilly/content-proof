"use client";

import { ReactNode } from "react";

interface StatusIndicatorProps {
  type: "success" | "error" | "warning" | "info" | "loading";
  title: string;
  message?: string;
  children?: ReactNode;
  className?: string;
  showIcon?: boolean;
  onClose?: () => void;
}

const statusConfig = {
  success: {
    containerClass: "status-success",
    iconBg: "bg-emerald-500",
    icon: (
      <svg
        className="w-4 h-4 text-white"
        fill="currentColor"
        viewBox="0 0 20 20"
      >
        <path
          fillRule="evenodd"
          d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
          clipRule="evenodd"
        />
      </svg>
    ),
  },
  error: {
    containerClass: "status-error",
    iconBg: "bg-red-500",
    icon: (
      <svg
        className="w-4 h-4 text-white"
        fill="currentColor"
        viewBox="0 0 20 20"
      >
        <path
          fillRule="evenodd"
          d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
          clipRule="evenodd"
        />
      </svg>
    ),
  },
  warning: {
    containerClass: "status-warning",
    iconBg: "bg-amber-500",
    icon: (
      <svg
        className="w-4 h-4 text-white"
        fill="currentColor"
        viewBox="0 0 20 20"
      >
        <path
          fillRule="evenodd"
          d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
          clipRule="evenodd"
        />
      </svg>
    ),
  },
  info: {
    containerClass: "status-info",
    iconBg: "bg-blue-500",
    icon: (
      <svg
        className="w-4 h-4 text-white"
        fill="currentColor"
        viewBox="0 0 20 20"
      >
        <path
          fillRule="evenodd"
          d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z"
          clipRule="evenodd"
        />
      </svg>
    ),
  },
  loading: {
    containerClass: "status-info",
    iconBg: "bg-blue-500",
    icon: (
      <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
    ),
  },
};

export default function StatusIndicator({
  type,
  title,
  message,
  children,
  className = "",
  showIcon = true,
  onClose,
}: StatusIndicatorProps) {
  const config = statusConfig[type];

  return (
    <div className={`${config.containerClass} animate-fade-in ${className}`}>
      {showIcon && (
        <div
          className={`w-8 h-8 rounded-full ${config.iconBg} flex items-center justify-center flex-shrink-0 shadow-lg`}
        >
          {config.icon}
        </div>
      )}

      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between">
          <div>
            <p className="font-bold text-base">{title}</p>
            {message && <p className="text-sm opacity-90 mt-1">{message}</p>}
          </div>

          {onClose && (
            <button
              onClick={onClose}
              className="ml-4 p-1 rounded-full hover:bg-black/10 transition-colors flex-shrink-0"
              aria-label="Close notification"
            >
              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                <path
                  fillRule="evenodd"
                  d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                  clipRule="evenodd"
                />
              </svg>
            </button>
          )}
        </div>

        {children && <div className="mt-3">{children}</div>}
      </div>
    </div>
  );
}
